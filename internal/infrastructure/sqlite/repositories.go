...
ORDER BY d.id`, "%"+strings.TrimSpace(search)+"%")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r NoteRepo) ListNotes(ctx context.Context, filters domain.FilterSet, page domain.Pagination) (result []domain.Note, err error) {
	...
	// 3.6: exact note id lookup (no deck join).
	if filters.NoteID > 0 {
	...
	// 3.7: global text search across notes.flds (no deck filter).
	if filters.DeckID == 0 && strings.TrimSpace(filters.SearchText) != "" {
		base := `SELECT DISTINCT n.id, n.guid, n.mid, n.flds, n.mod, n.usn FROM notes n WHERE n.flds LIKE ? COLLATE NOCASE`
		args := []any{"%" + filters.SearchText + "%"}
		...
		base += " ORDER BY n.id LIMIT ? OFFSET ?"
	...
	base := `
SELECT DISTINCT n.id, n.guid, n.mid, n.flds, n.mod, n.usn
FROM notes n
...
WHERE c.did = ?`
	...
	if filters.SearchText != "" {
		base += " AND n.flds LIKE ? COLLATE NOCASE"
...
func (r NoteRepo) UpdateNote(ctx context.Context, note domain.Note) error {
	q := queryer(ctx, r.db)
	_, err := q.ExecContext(ctx, "UPDATE notes SET flds = ?, mod = ?, usn = ? WHERE id = ?", note.RawFlds, note.Mod, note.USN, note.ID)
	return err
}
...
func (r ModelRepo) GetModelByID(ctx context.Context, modelID int64) (domain.Model, error) {
	q := queryer(ctx, r.db)
	model, err := r.getModelFromFieldsTable(ctx, q, modelID)
	if err == nil {
		return model, nil
	}
	model, err = r.getModelFromModelsTable(ctx, q, modelID)
	if err == nil {
		return model, nil
	}
